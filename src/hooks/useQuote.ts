...
export const STORAGE_KEY_QUOTE_DRAFT = 'quote_gen_data';
export const STORAGE_KEY_QUOTE_ACTIVE_HISTORY_ID = 'quote_gen_active_history_id';
export const STORAGE_KEY_QUOTE_ORIGINAL_SNAPSHOT = 'quote_gen_original_snapshot';

export const useQuote = (initialSettings?: Settings) => {
  const [data, setData] = useState<QuoteData>(() => {
    ...
    const hydrated = loadHydratedData<QuoteData>(defaultWithSettings, STORAGE_KEY_QUOTE_DRAFT);
  ...
  const setOriginalSnapshotForCurrentData = useCallback((newData: QuoteData, newHistoryId: string | null) => {
  ...
  const loadQuoteFromHistory = useCallback((historyId: string, historyData: QuoteData) => {
    setData(historyData);
    setOriginalSnapshotForCurrentData(historyData, historyId);
  }, [setOriginalSnapshotForCurrentData]);

  const updateBusiness = useCallback((business: Partial<QuoteData['business']>) => {
    setData(prev => ({ ...prev, business: { ...prev.business, ...business } }));
  }, []);

  const updateClient = useCallback((client: Partial<QuoteData['client']>) => {
    setData(prev => ({ ...prev, client: { ...prev.client, ...client } }));
  }, []);

  const updateDetails = useCallback((details: Partial<QuoteData['details']>) => {
    setData(prev => ({ ...prev, details: { ...prev.details, ...details } }));
  }, []);

  const updateOtherFields = useCallback((updates: Partial<QuoteData['totals']>) => {
  ...
  const updateTotals = useCallback((totalsUpdate: Partial<QuoteData['totals']>) => {
  ...
  const resetData = useCallback((skipSequenceReset?: boolean) => {
    ...
    setData({
      ...
      details: {
        ...defaultQuote.details,
        quoteNumber: skipSequenceReset ? defaultQuote.details.quoteNumber : generateQuoteNumber()
      }
    });
  }, []);
  ...
  const startNewRevision = useCallback(() => {
    const newPoNum = generateQuoteNumber();
    setData(prev => ({
      ...prev,
      details: {
        ...prev.details,
        quoteNumber: newPoNum
      }
    }));
    try {
      localStorage.removeItem(STORAGE_KEY_QUOTE_ACTIVE_HISTORY_ID);
  ...
  return {
    ...
    loadedHistoryId,
    loadInvoiceFromHistory: loadQuoteFromHistory,
    setOriginalSnapshotForCurrentData,
    isDirty
  };
};
