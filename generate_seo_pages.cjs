 ...
 * Creates 9 keyword-targeted landing pages with unique content, FAQ, and JSON-LD
 */
const fs = require('fs');
...
function generatePage(page) {
  const url = `https://invoice-gen.net/${page.slug}/`;
  
  const faqSchemaItems = page.faqs.map(faq => `
            {
              "@type": "Question",
...
<head>
  ...
  <meta name="twitter:image" content="https://invoice-gen.net/og-image.webp" />
    ...
    "@graph": [
      {
        ...
        "url": "${url}",
        "isPartOf": { "@id": "https://invoice-gen.net/#website" },
        "publisher": { "@id": "https://invoice-gen.net/#organization" }
      },
      {
        "@type": "BreadcrumbList",
        "itemListElement": [
          { "@type": "ListItem", "position": 1, "name": "Home", "item": "https://invoice-gen.net/" },
          { "@type": "ListItem", "position": 2, "name": "${page.h1.split(' — ')[0]}", "item": "${url}" }
        ]
      },
      {
        "@type": "FAQPage",
  ...
  <style>
    :root { --color-primary: #4F46E5; --color-primary-hover: #4338CA; --color-text-main: #0F172A; --color-text-secondary: #475569; --color-border: #E2E8F0; --color-bg: #FAFCFF; }
    * { box-sizing: border-box; margin: 0; padding: 0; }
    body { font-family: 'Inter', -apple-system, BlinkMacSystemFont, sans-serif; color: var(--color-text-main); background-color: var(--color-bg); line-height: 1.6; -webkit-font-smoothing: antialiased; }
    ...
    .internal-links a { color: var(--color-primary); text-decoration: none; font-size: 14px; font-weight: 500; background: rgba(79,70,229,0.06); padding: 8px 16px; border-radius: 20px; transition: all 150ms; }
    .internal-links a:hover { background: rgba(79,70,229,0.12); }
    footer { background: #0F172A; color: #94A3B8; padding: 40px 0; text-align: center; font-size: 14px; margin-top: 60px; }
    footer a { color: #94A3B8; text-decoration: none; margin: 0 12px; }
    footer a:hover { color: white; }
    @media (max-width: 768px) { h1 { font-size: 28px; } main { padding: 40px 16px; } .cta-box { padding: 28px 20px; } }
  </style>
</head>
<body>
  <header>
  ...
  <main>
    <article>
      ...
      <h2>Key Features</h2>
      <ul>
${page.features.map(f => `        <li><strong>${f.split(' — ')[0]}</strong></li>`).join('\n')}
      </ul>
...
${page.faqs.map(faq => `      <div class="faq-item">
        <h3>${faq.q}</h3>
        <p>${faq.a}</p>
      </div>`).join('\n')}
      ...
      <div class="internal-links">
        ...
        <a href="/templates/consultant/">Consultant Invoice Template</a>
        <a href="/blog/how-to-get-paid-faster-invoices/">How to Get Paid Faster</a>
      </div>
    </article>
  </main>
...
// Generate all pages
console.log('🔄 Generating programmatic SEO landing pages...\n');

let count = 0;
for (const page of pages) {
  const dir = path.join(__dirname, 'public', page.slug);
...
console.log(`\n✅ Generated ${count} landing pages successfully.`);
